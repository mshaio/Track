...
class Analyse:
    ...
    def get_gradient(self):
        if self.file:
            ...
            for i in range(len(x_coords) - 1):
                if i+1 <= len(x_coords):
                    if (y_coords[i+1] - y_coords[i]) == 0 or (x_coords[i+1] - x_coords[i]) == 0:
        ...
        self.get_data_from_postgres()
        for index, data in enumerate(self.data):
            if index+1 < len(self.data):
                if (self.data[index+1]['y_coord'] - self.data[index]['y_coord']) == 0 or (self.data[index+1]['x_coord'] - self.data[index]['x_coord']) == 0:
                    self.gradient.append(0)
                else:
                    gradient = (self.data[index+1]['y_coord'] - self.data[index]['y_coord'])/(self.data[index+1]['x_coord'] - self.data[index]['x_coord'])
                    self.gradient.append(gradient)
        return self.gradient
    ...
    def get_movement_direction(self):
        ...
        if self.file:
            ...
            x_coords = [data_point['x_coord'] for data_point in self.data]
            y_coords = [data_point['y_coord'] for data_point in self.data]
